...
| Plain Rust cdylib installs no signal handlers; audit glue-crate deps for sigaction | test | `tools/rulisp-audit.sh` (signal-symbol sweep on the artifact, tokio signal/process features, `block_on`), run over all examples by `make audit` in CI; self-tested against `tools/audit-fixture` |
