...
| Handle stamped with birth generation; accepted only by wrappers of that same birth generation | runtime-check | `lisp/src/handle.lisp:30-52` (`cell-begin-call` gate); `lisp/src/codegen.lisp:400-406,316-326`; test `m6.captured-wrapper-gate`. The generation counter is not writable through the exported API: `crate-generation` is a reader (v0.6 — an exported setf once let a stale handle into a new library); test `v06.crate-generation-is-read-only` |
...
| **§7 — Threads and signals** | | |
| Any Lisp thread may call any export; execution on the calling thread; no global lock, no runtime | test | `m4h.thread-race` (`tests/suite/m4.lisp:18-53`); also `fetch.concurrent-lisp-threads` (`tests/suite/fetch.lisp`; CI: SBCL/Linux and CCL/Linux required jobs) |
