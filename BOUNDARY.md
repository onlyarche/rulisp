...
| Breaking the contract requires bumping `abi_version()`; loader requires exact match | runtime-check | `lisp/src/crate.lisp:203-205` (`abi-mismatch-error` unless `= +abi-version+`) |
| Adding manifest keys is NOT a break — unknown keys are ignored | test | `fx.unknown-keys-ignored` (`tests/suite/m2.lisp:57-61`) |
| `tests/golden/` pins the manifest bytes (macro output byte-identical) | test | `fx.golden-manifest` (`tests/suite/m2.lisp:125-146`) |
| **§1 — Symbols and per-library resolution** | | |
| Every export is prefixed `<crate>_rulisp_` (crate `-` → `_`) | runtime-check | `lisp/src/crate.lisp:236-241` (`manifest-error` if prefixed symbol absent); prefix minted `crates/rulisp-macros/src/lib.rs:23-29` |
| Loader resolves every symbol against the specific dlopen handle, never the global namespace | runtime-check | `lisp/src/ffi.lisp:95-98` (`dlsym-ptr`); `%find-symbol-in` `ffi.lisp:58-59` |
| After a reload two generations export identical names yet stay distinct per wrapper/handle | test | `m6.reload` + `m6.captured-wrapper-gate` (`tests/suite/m1.lisp:245,259`) |
...
| `abi_version()` is checked first, before the manifest is read; must equal 1 | runtime-check | `lisp/src/crate.lisp:194-205` (`%open-and-verify`) |
| `manifest()` is a static permanent borrow (`'static str` behind OnceLock/static) | compile-error | `crates/rulisp-macros/src/lib.rs:1380-1407`; oracle `tests/m1-handwritten/src/lib.rs:18,38-43` |
| Caller copies the manifest and never frees it | runtime-check | `lisp/src/crate.lisp:219-228` (`%read-library-manifest`: copies, no dealloc) |
| `last_error` and `dealloc` entry points exist per library (resolved at load) | runtime-check | `lisp/src/crate.lisp:242-243` (resolve signals `manifest-error` if missing) |
| **§2 — Status codes** | | |
| Every function shim returns `int32_t` | compile-error | `crates/rulisp-macros/src/lib.rs:746-756` (generated shim signature `-> i32`) |
| Out-params are written only on OK | test | `v04.out-params-untouched-on-err` (`tests/suite/v04.lisp:117-147`) |
| OK (0): out-params valid; CL converts and returns | test | `m3.strings` (`tests/suite/m1.lisp:102`); dispatch `lisp/src/codegen.lisp:545-549` |
...
| Status 3 → CL signals `rulisp:invalid-argument` | runtime-check | `lisp/src/codegen.lisp:589-593` |
| CB_ERR (4): CL re-signals the original stashed condition object | test | `m5.callback-condition-identity` (`tests/suite/m1.lisp:202-215`) |
| Exception: `*_free` returns void; a panic inside free is caught, logged to stderr, swallowed | test | `v04.free-shim-swallows-drop-panic` (`tests/suite/v04.lisp:53-67`); shim `crates/rulisp-macros/src/lib.rs:914-922` |
| **§3 — last_error** | | |
...
| CL reads `last_error` through the owning generation's resolved pointer | runtime-check | `lisp/src/codegen.lisp:574-593` (`gen-ctx-last-error-ptr`); resolved `lisp/src/crate.lisp:242` |
| Borrowed until next call into same library on same thread; CL copies both strings immediately | runtime-check | `lisp/src/ffi.lisp:315-326` (`read-crate-last-error` copies; called from `codegen.lisp:577-590`) |
...
| Panics: type `"panic"`, msg is the payload when `&str`/`String` | test | `m1.panic` (`tests/suite/m1.lisp:57-62`); `crates/rulisp-runtime/src/lib.rs:111-126` |
| Non-string panic payload → msg `"panic (non-string payload)"` | runtime-check | `crates/rulisp-runtime/src/lib.rs:115-122` (branch untested) |
| UTF-8 rejection records type `"InvalidUtf8"` | runtime-check | `crates/rulisp-runtime/src/lib.rs:165` |
| **§4 — Strings and buffers** | | |
...
| Lisp→Rust: borrowed for the duration of the call (dynamic-extent pin/copy) | runtime-check | `lisp/src/ffi.lisp:213-238,267-273` (`call-with-bytes-arg` / `call-with-utf8-arg`) |
...
| Rust validates UTF-8; failure → status 3 | runtime-check | `crates/rulisp-runtime/src/lib.rs:154-168` (`str_arg`) |
| Retention layer 1: macro rejects any explicit lifetime in an export signature | compile-error | `crates/rulisp-macros/src/lib.rs:123-164,1003-1011`; pinned by `crates/rulisp/tests/ui/static_lifetime.rs` (trybuild) |
| Retention layer 2: borrowing helpers pin the returned lifetime to a per-call ShimFrame | compile-error | `crates/rulisp-runtime/src/lib.rs:136-168` (`ShimFrame` + `str_arg<'a>(&'a ShimFrame)`) |
| Both layers cover byte buffers, scalar slices, handle references and borrowed callbacks | compile-error | `crates/rulisp-runtime/src/lib.rs:208-261`; `crates/rulisp/src/lib.rs:222-232`; `ui/static_lifetime.rs:22-56` |
...
| Strings have align 1; `len == capacity` guaranteed by construction | runtime-check | `crates/rulisp-runtime/src/lib.rs:174-187` (`into_boxed_slice`); align-1 dealloc `lisp/src/ffi.lisp:337-339` |
...
| len-0 dangling pointer never dereferenced; size-0 dealloc is a no-op | runtime-check | `lisp/src/ffi.lisp:171-181,203-211,328-335`; `crates/rulisp-runtime/src/lib.rs:237-240` |
...
| **§5 — Handles** | | |
| Handle is `void*` = `Box::into_raw(Box<T>)`; constructors return it via `void **out` | runtime-check | `crates/rulisp-runtime/src/lib.rs:249-251` (`handle_new`); `crates/rulisp-macros/src/lib.rs:650-653` (`out: *mut *mut c_void`) |
| Method shims reborrow `const void*` → `&T` | runtime-check | `crates/rulisp-runtime/src/lib.rs:259-261` (`handle_ref`); `crates/rulisp-macros/src/lib.rs:562-571` |
| No `&mut` across the boundary and no by-value consumption | compile-error | `crates/rulisp-macros/src/lib.rs:170-176` (&mut param), `1013-1031` (&mut/by-value self); `crates/rulisp/tests/ui/mut_self.rs` |
| `T: Send + Sync + 'static` enforced at compile time (HandleType supertrait bounds) | compile-error | `crates/rulisp/src/lib.rs:107`; impl emitted at `crates/rulisp-macros/src/lib.rs:895-901`; `crates/rulisp/tests/ui/non_send_handle.rs` |
| Free shim: NULL is a no-op | runtime-check | `crates/rulisp-runtime/src/lib.rs:267-271` (`handle_free` null check) |
| A non-NULL double free is undefined behavior at the C level | UB-by-design | `BOUNDARY.md:93-95`; safety contract `crates/rulisp-runtime/src/lib.rs:263-266` |
| Exactly-once free via CL cell state machine; free racing in-flight call accepted, deferred, run by last call | runtime-check | `lisp/src/handle.lisp:54-97` (`cell-end-call`, `%cell-free`); tests `m4.handle-lifecycle`, `m4.free-vs-in-flight` (`tests/suite/m1.lisp:124-174`) |
| Handle stamped with birth generation; accepted only by wrappers of that same birth generation | runtime-check | `lisp/src/handle.lisp:30-52` (`cell-begin-call` gate); `lisp/src/codegen.lisp:497-503,342-352`; test `m6.captured-wrapper-gate`. The generation counter is not writable through the exported API: `crate-generation` is a reader (v0.6 — an exported setf once let a stale handle into a new library); test `v06.crate-generation-is-read-only` |
| Handle always freed by its birth generation's free shim | runtime-check | `lisp/src/codegen.lisp:342-352` (gen-ctx free-table); `lisp/src/handle.lisp:65,91` (`cell-free-fn`); test `m6.reload` (`tests/suite/m1.lisp:254`) |
| **§6 — Callbacks (wire + borrowed)** | | |
| Fn-ptr param + reserved uint64 userdata (0 in v1); trampoline returns 0 OK / 1 condition-stashed | runtime-check | `lisp/src/codegen.lisp:132-165` (returns), `:517` (`:uint64 0`); `crates/rulisp-macros/src/lib.rs:573-586` |
| §6.1 Callback invoked only on calling thread before export returns; `!Send`/`!Sync` + lifetime-pinned; storing = compile error | compile-error | `crates/rulisp/src/lib.rs:166-170` (`PhantomData *mut ()`), `217-232` (ShimFrame pin); `crates/rulisp/tests/ui/stored_callback.rs`, `crates/rulisp/tests/ui/static_lifetime.rs` |
| §6.2 Lisp condition never unwinds through Rust: trampoline catches serious-condition, stashes it, returns 1 | runtime-check | `lisp/src/codegen.lisp:145-159`; test `m5.callback-condition-identity` |
| §6.2 Rust sees `Err(CallbackError)` and unwinds normally: destructors run | test | `m5.callback-condition-identity` (`tests/suite/m1.lisp:202-215`, guard-drops assertion `:214`); Err mapping `crates/rulisp/src/lib.rs:237-245` |
| §6.2 Propagated with `?`: shim reports status 4; CL wrapper re-signals the original condition object | runtime-check | `crates/rulisp-macros/src/lib.rs:710-743` (CB tunnel); `lisp/src/codegen.lisp:594-598`; identity tested by `m5.callback-condition-identity`, `m4h.nested-callbacks` |
| §6.3 Rust swallows CallbackError and returns Ok: stash discarded (dynamic binding scope ends) | test | `v04.swallowed-callback-error-is-discarded` (`tests/suite/v04.lisp:14-26`); binding `lisp/src/codegen.lisp:511-514` |
| §6.3 Swallow-then-fail with a stashed condition is conservatively status 4; original condition re-signaled | runtime-check | `crates/rulisp-macros/src/lib.rs:731-743` (tunnel_check on Err branch); test `v04.swallow-then-fail-resignals-the-original` (`tests/suite/v04.lisp:35-46`) |
| §6.4 Non-local Lisp exit unwinds through Rust frames skipping destructors; trampoline logs best-effort warning | UB-by-design | `BOUNDARY.md:124-128`; warning at `lisp/src/codegen.lisp:160-165` |
| **§6 — Stored callbacks** | | |
| Same wire, different trampoline; leading uint64 carries registry id minted by `rulisp:callback`; token keeps closure registered | runtime-check | `lisp/src/stored-callback.lisp:42-58`; `lisp/src/codegen.lisp:518-532` (token-id in `:uint64` slot) |
| Storable, Clone/Copy, Send+Sync; Rust may invoke from any thread (Lisp adopts foreign threads; SBCL/CCL) | test | `v02.stored-callback-cross-thread` (`tests/suite/v02.lisp:118-137`); derives `crates/rulisp/src/lib.rs:183-188` |
| Unregistered/GC'd token: invocation returns error status after Lisp-side warning; dead id never dangles | runtime-check | `lisp/src/codegen.lisp:205-211` (lookup nil → warn, status 2); test `v02.stored-callback-dead-id` (`tests/suite/v02.lisp:139-152`) |
| Signaled condition is warned, reported `Err(CallbackError)` status 1 (dead id 2); no stash, no re-signal, no status 4 | runtime-check | `lisp/src/codegen.lisp:220-223`; no tunnel in `crates/rulisp/src/lib.rs:206-214`; test `v02.stored-callback-condition` (`v02.lisp:154-170`) |
| Non-local-exit UB clause applies identically to stored callbacks | UB-by-design | `BOUNDARY.md:145`; warning at `lisp/src/codegen.lisp:224-226` |
| **§7 — Threads and signals** | | |
| Any Lisp thread may call any export; execution on the calling thread; no global lock, no runtime | test | `m4h.thread-race` (`tests/suite/m4.lisp:63-104`); also `fetch.concurrent-lisp-threads` (`tests/suite/fetch.lisp`; CI: SBCL/Linux and CCL/Linux required jobs) |
| Concurrent method calls on one handle allowed (`T: Sync` carries the load) | compile-error | `crates/rulisp/src/lib.rs:107` (Sync supertrait); `crates/rulisp/tests/ui/non_send_handle.rs`; exercised by `m4h.thread-race` |
| Free-vs-in-flight races resolved by deferral (restates §5) | test | `m4.free-vs-in-flight` (`tests/suite/m1.lisp:162-174`); mechanism `lisp/src/handle.lisp:54-97` |
| Rust internal threads cannot call borrowed Lisp callbacks (`!Send`) | compile-error | `crates/rulisp/src/lib.rs:166-169` (`PhantomData *mut ()`); `crates/rulisp/tests/ui/stored_callback.rs` |
| Blocking exports must cap their wait in Rust; the loop belongs in Lisp (glue-author norm) | test | `fetch.waits-are-capped` (`tests/suite/fetch.lisp`; CI: SBCL/Linux and CCL/Linux required jobs); cap `examples/fetch/src/lib.rs:42-47` — reference crate only, unenforceable for third-party glue |
| Blocking export must refuse re-entry from its own runtime's worker thread, reported as an error | runtime-check | `examples/fetch/src/lib.rs:52-59` (`refuse_reentry`; call sites 331,347,612,650) — reference crate only; no test can trigger it from Lisp |
| ECL: callback trampolines natively compiled at load time; missing C toolchain fails via named `manifest-error` | runtime-check | `lisp/src/codegen.lisp:105-124` (`#+ecl` compile-file branch); test `v04.ecl-toolchain-failure-is-named` (`tests/suite/v04.lisp:156-166`, non-vacuous on ECL only) |
| ECL: foreign-thread invocation of stored callbacks UNSUPPORTED (cannot adopt foreign threads) | UB-by-design | `BOUNDARY.md:165-171`; `docs/upstream/ecl-dynamic-callback-gc.md`; `#+ecl` skip `tests/suite/v02.lisp:118-124` |
...
| Lisp side owns SIGINT/SIGTERM; a glue crate's artifact imports no signal-disposition symbol | test | `tools/rulisp-audit.sh` over every example (`make audit`, CI), with `tools/rulisp-audit-selftest.sh` proving the gate rejects a signal-importing fixture | fetch demonstrates the pattern (`Client::shutdown` `examples/fetch/src/lib.rs:346`; audit.sh bans signal symbols) but nothing enforces the norm generally |
| **§8 — Panics and non-local exits** | | |
| Every shim body runs under `catch_unwind`; a panic maps to status 2 | runtime-check | `crates/rulisp-runtime/src/lib.rs:111-126` (`rt::shim`); every generated shim wrapped `crates/rulisp-macros/src/lib.rs:746-756`; test `m1.panic` |
...
| Shims are `extern "C"` not `"C-unwind"`: an escaped panic aborts rather than corrupting the caller | runtime-check | `crates/rulisp-macros/src/lib.rs:748,915,1397-1420` (all shims/entry points `extern "C"`; abort-on-escape is rustc's extern-C backstop) |
| Poisoned mutexes after a caught panic are the glue crate's own concern (rulisp adds no mitigation) | UB-by-design | `BOUNDARY.md:190-192` |
| Subsequent `.lock().unwrap()` on a poisoned mutex panics, is caught, reported as status 2 | test | `v04.poisoned-mutex-reports-panic` (`tests/suite/v04.lisp:74-89`; oracle `word_bag_poison` `tests/m1-handwritten/src/lib.rs:475-482`) |
| **§9 — Reload and unloading** | | |
| The loader never calls dlclose; old generations stay mapped forever | test | `m6.captured-wrapper-gate` (`tests/suite/m1.lisp:259-279`): old-gen wrapper+handle callable post-reload (`:273`); no dlclose/FreeLibrary anywhere in `lisp/src` |
| One leaked mapping per reload (zero in production) — a deliberate, accepted leak | UB-by-design | `BOUNDARY.md:196-198`; `lisp/src/crate.lisp:3-6` |
...
| Every captured wrapper closure of a dead session refuses (`crate-not-loaded-error`) | runtime-check | `lisp/src/codegen.lisp:566-571`; pinned by `m7.dump-restore` (`tests/suite/m1.lisp:335-339`) |
| Then each crate reloads from its recorded artifact path and all bindings regenerate | test | `m7.dump-restore` (`tests/suite/m1.lisp:326-327`: post-restore GREET works); mechanism `lisp/src/crate.lisp:472-483` |
...
| Freeing a dead-session handle performs no foreign call | runtime-check | `lisp/src/handle.lisp:69-75` (`%maybe-foreign-free` session gate); m7 exercises explicit free + GC-finalizer path (`m1.lisp:333,340-345`) |
| No guardrail against dumping with live foreign threads; they vanish on restore, their state gone | UB-by-design | `BOUNDARY.md:216-221` |
...
| `:schema` accepted only when <= supported form version (1) | runtime-check | `lisp/src/manifest.lisp:128-131`; test `fx.newer-schema-refused` (`tests/suite/m2.lisp:35-37`) |
| `:abi` must equal `abi_version()` exactly (loader exact-match + manifest/entry-point agreement) | test | `v06.abi-mismatch-refused` (`tests/suite/v06.lisp`; `tests/abi-fixture` answers 2 and is refused with expected 1, actual 2, nothing registered); mechanism `%open-and-verify` (`lisp/src/crate.lisp`) |
| `:target` checked against the host; unknown tokens pass | runtime-check | `lisp/src/manifest.lisp:106-121` + `lisp/src/crate.lisp:210-216`; test `fx.target-check` (`tests/suite/m2.lisp:71-78`) |
| Required keys and well-formed specs; any corruption signals a named `manifest-error` | runtime-check | `lisp/src/manifest.lisp:51-61,123-152`; tests `fx.missing-required-keys`, `fx.not-a-manifest`, `fx.bad-param-form` (`tests/suite/m2.lisp`) |
| `:errors` names become condition classes, subclasses of `rulisp:rust-error` | test | `m3.typed-conditions` (`tests/suite/m3.lisp:11-28`); mechanism `lisp/src/crate.lisp:249-251,333-335` |
...
| Type vocabulary is closed: tokens outside it signal `manifest-error` at binding generation | runtime-check | `lisp/src/codegen.lisp:40-42,80-86,167-175`; test `fx.partial-generation-ban` (`m2.lisp:96-118`); macro side: trybuild `ui/bad_type.rs` |
| `(:option T)`: leading uint8 present flag; value meaningful only when present; Lisp NIL ↔ None | test | `v02.option-result` + `v02.option-param` (`tests/suite/v02.lisp:52-62`); mechanism `codegen.lisp:274-295,456-476`, `ffi.lisp:275-285` |
| `(:option T)` with `:bool` inner is rejected (nil ambiguity) | compile-error | `crates/rulisp-macros/src/lib.rs:226-230` (param), `359-363` (result) |
...
| `(:vec S)`: S scalar only; len counts ELEMENTS; freed via `dealloc(ptr, len*sizeof(S), alignof(S))` | runtime-check | `lisp/src/codegen.lisp:80-86,326-340`; `crates/rulisp-runtime/src/lib.rs:189-200`; test `v02.vec-roundtrip` (`v02.lisp:69-81`) |
| `:bytes` = exact `:string` convention minus UTF-8 validation; empty transfer at len 0 | test | `v02.bytes-in`/`bytes-roundtrip`/`bytes-alloc-pairing` (`tests/suite/v02.lisp:10-45`); no-validation path `crates/rulisp-runtime/src/lib.rs:216-228` |

## Verification notes

- Every `file:line` above was read and confirmed against the working tree — first in v0.4, and again in v0.6 (item 14: 110 line ranges had drifted with the code and were re-anchored; every named test still exists). Two citations in the input were corrected: the oracle manifest static (`tests/m1-handwritten/src/lib.rs:17,37-42`, was 18,40-43) and the stored-callback dead-id branch (`codegen.lisp:179-185`, was 179-186).
