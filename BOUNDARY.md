...
| `T: Send + Sync + 'static` enforced at compile time (HandleType supertrait bounds) | compile-error | `crates/rulisp/src/lib.rs:58`; impl emitted at `crates/rulisp-macros/src/lib.rs:895-901`; `crates/rulisp/tests/ui/non_send_handle.rs` |
...
| Handle always freed by its birth generation's free shim | runtime-check | `lisp/src/codegen.lisp:342-352` (gen-ctx free-table); `lisp/src/handle.lisp:65,91` (`cell-free-fn`); test `m6.reload` (`tests/suite/m1.lisp:243`) |
...
| §6.2 Rust sees `Err(CallbackError)` and unwinds normally: destructors run | test | `m5.callback-condition-identity` (`tests/suite/m1.lisp:191-204`, guard-drops assertion `:214`); Err mapping `crates/rulisp/src/lib.rs:237-245` |
...
| **§8 — Panics and non-local exits** | | |
| Every shim body runs under `catch_unwind`; a panic maps to status 2 | runtime-check | `crates/rulisp-runtime/src/lib.rs:111-126` (`rt::shim`); every generated shim wrapped `crates/rulisp-macros/src/lib.rs:719-729`; test `m1.panic` |
